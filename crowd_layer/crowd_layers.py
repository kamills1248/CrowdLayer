...
class MaskedMultiBinaryCrossEntropy(object):
    ...
    def loss(self, y_true, y_pred):
        vec = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=y_pred, labels=y_true), axis=1,
                             keepdims=False)
        mask = tf.equal(y_true[:, 0, :], -1)
        zer = tf.zeros_like(vec)
        loss = tf.compat.v1.where(mask, x=zer, y=vec)
...
class CrowdsAggregationCategoricalCrossEntropy(object):
    ...
    def m_step(self):
        # print "M-step"
        self.pi_est = tf.transpose(
            a=self.suff_stats / tf.expand_dims(tf.reduce_sum(input_tensor=self.suff_stats, axis=2), 2), perm=[1, 2, 0])
...
class CrowdsAggregationBinaryCrossEntropy(object):
    ...
    def loss_fc(self, y_true, y_pred):
        ...
        for r in range(self.num_annotators):
            ...
            b = b * tf.compat.v1.where(tf.equal(y_true[:, r], 1), (1.0 - self.beta[r]) * tf.ones_like(p),
                                       tf.ones_like(p))
            a = a * tf.compat.v1.where(tf.equal(y_true[:, r], 0), (1.0 - self.alpha[r]) * tf.ones_like(p),
                                       tf.ones_like(p))
        ...
        # update suff stats
        for r in range(self.num_annotators):
            self.suff_stats_alpha[r] += tf.reduce_sum(
                input_tensor=tf.compat.v1.where(tf.equal(y_true[:, r], 1), mu, tf.zeros_like(p)))
            self.suff_stats_beta[r] += tf.reduce_sum(
                input_tensor=tf.compat.v1.where(tf.equal(y_true[:, r], 0), (1.0 - mu), tf.zeros_like(p)))
            self.suff_stats_alpha_norm[r] += tf.reduce_sum(
                input_tensor=tf.compat.v1.where(tf.equal(y_true[:, r], -1), tf.zeros_like(p), mu))
            self.suff_stats_beta_norm[r] += tf.reduce_sum(
                input_tensor=tf.compat.v1.where(tf.equal(y_true[:, r], -1), tf.zeros_like(p), (1.0 - mu)))
